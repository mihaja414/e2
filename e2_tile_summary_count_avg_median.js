...
(async () => {
        ...
        getWaitTime (index, defaultWaitTime) {
            ...
            if (index > 0) {
                ...
                if (this.isModN(index, 10)) {
                    ...
                    if (this.isModN(index, 20) || this.isModN(index, 50)) {
                        result = 2048;
                    }
    ...
    class E2API {
        ...
        init () {
            ...
            this.apiClient = ___reactContext.api.apiClient;
            this.itemsPerPage = 100; //60;
        }

        async getAllProperties () {
            let firstPage = await this.getPropertyPage(1);
            //console.log("first page: ", firstPage);

            let total = firstPage.count;
            let pageCount = Math.ceil(total / this.itemsPerPage);

            let result = firstPage.results;
            for (let i = 2; i <= pageCount; i++) {
                console.log(`query: [${i}/${pageCount}]`);
                let pageData = await this.getPropertyPage(i);
                //console.log("page data: ", pageData);
                result = result.concat(pageData.results);
                await helper.sleep(helper.getWaitTime(i, 255));
            }

            for (let i = 0; i < result.length; i++) {
                this.matchCountry(result[i]);
            }

            console.log("getting all properties finished");
            return result;
        }

        async getPropertyPage (pageNumber) {
            //return await this.apiClient.get(`/landfields?items=${this.itemsPerPage}&page=${pageNumber}&userId=${window.auth0user.id}`);
            return await fetch(`/api/v2/my/landfields/?limit=${this.itemsPerPage}&offset=${(pageNumber - 1) * this.itemsPerPage}`).then(r => r.json());
        }

        async getAllPropertiesForUser (userId) {

            let firstPage = await this.getPropertyPageForUser(userId, 1);
            console.log("first page: ", firstPage);

            let total = firstPage.count;
            let pageCount = Math.ceil(total / 60);

            let result = firstPage.landfields;
            for (let i = 2; i <= pageCount; i++) {
                console.log(`query: [${i}/${pageCount}]`);
                let pageData = await this.getPropertyPageForUser(userId, i);
                //console.log("page data: ", pageData);
        ...
        async getPropertyPageForUser (userId, pageNumber) {
            let result = await ___reactContext.api.apiClient.get(`/landfields?items=60&page=${pageNumber}&search=&userId=${userId}`);
            return result.data;
        }

        async getCountryData () {
            fetch("/api/v2/tileprices/")
                .then(r => r.json())
                .then(r => {
                    this.countryData = r;
                    console.log("[LOG]: got country data");
                })
                .catch(e => console.log("getCountryData error: ", e));
        }

        matchCountry (property) {
            let locationParts = property["location"].split(", ");
            //console.log("matchCountry ", { property, locationParts });
            let countryName = locationParts[locationParts.length - 1];
            property.countryName = countryName;

            property.correctedLocation = locationParts.reverse().join(" | ");
        }

        async getPropertyInfoShort (propertyId) {

            if (!this.propertyCache.has(propertyId)) {
                let req = await ___reactContext.api.apiClient.get(`/landfields/${propertyId}`);
                this.propertyCache.set(propertyId, req.data);
                await helper.sleep(1024);
            }
    ...
    let useClassData = true;

    if (useClassData) {
        window.allProperties = await api.getAllProperties();
    } else {
        window.allProperties = await api.getAllPropertiesForUser(auth0user.id);
    }

    let tier1 = window.allProperties.filter(p => p.landfield_tier === 1);
    let tier2 = window.allProperties.filter(p => p.landfield_tier === 2);
    let t1c1 = tier1.filter(p => p.tile_class === 1);
    let t1c2 = tier1.filter(p => p.tile_class === 2);
    let t1c3 = tier1.filter(p => p.tile_class === 3);
    let t1c4 = tier1.filter(p => p.tile_class === 4);
    let t1c5 = tier1.filter(p => p.tile_class === 5);

    if (tier1.length > 0) {
        logTiles(tier1, 1, undefined, "deeppink");
        logTiles(t1c1, 1, 1);
        logTiles(t1c2, 1, 2);
        logTiles(t1c3, 1, 3);
        logTiles(t1c4, 1, 4);
    ...
    if (tier2.length > 0) {
        logTiles(tier2, 2, undefined, "deeppink");
    }



    function logTiles (tileArray, tileTier, tileClass, totalColor) {
        let tileCounts = tileArray.map(m => m.tile_indexes.length);
        if (tileCounts.length > 0) {
            let total = tileCounts.reduce((partialSum, b) => partialSum + b, 0);
            let average = total > 0 ? Math.floor(total / tileCounts.length) : 0;
            let medianCount = Math.floor(median(tileCounts));

            let flexCount = tileCounts.filter(m => m === 750).length;

            total = `${total}`.padStart(7, " ");
            average = `${average}`.padStart(4, " ");
            medianCount = `${medianCount}`.padStart(4, " ");

            let prefix = getPrefix(tileTier, tileClass);
            console.log(`%c${prefix} total:%c ${total} %c| average: %c${average} %c| median: %c${medianCount} %c | flex count: %c${flexCount}`
                , "color:snow; border-bottom: 1px dashed lawngreen; padding-bottom:0.25em;"
                , `color:${totalColor ?? "goldenrod"}`
                , "color:snow;"
                , "color:lawngreen;"
                , "color: snow;"
                , "color:deepskyblue;"
    ...
    function getPrefix (tileTier, tileClass) {
        let prefix = `tier${tileTier}`;
        if (tileClass) {
            prefix = `T${tileTier}:C${tileClass}`;
        }
        return prefix;
    }
